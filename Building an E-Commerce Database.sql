...
CREATE TABLE shopping.BankCard
(
...
CREATE TABLE shopping.CreditCard
(
    cardNumber VARCHAR(25) NOT NULL
    ,userid INT NOT NULL
...
CREATE TABLE shopping.DebitCard
(
    cardNumber VARCHAR(25) NOT NULL
    ,userid INT NOT NULL
    ,PRIMARY KEY(cardNumber)
...
CREATE TABLE shopping.Comments  -- Weak Entity
(
...
CREATE TABLE shopping.ServicePoint
(
    spid INT NOT NULL
...
CREATE TABLE shopping.Save_to_Shopping_Cart
(
    userid INT NOT NULL
    ,pid INT NOT NULL
    ,addTime DATE
...
CREATE TABLE shopping.Deliver_To
(
    addrid INT NOT NULL
    ,orderNumber INT NOT NULL
    ,TimeDelivered DATE
...
CREATE TABLE shopping.After_Sales_Service_At
(
    brandName VARCHAR(20) NOT NULL
    ,spid INT NOT NULL
...
-- users with their credit card --

SELECT u.name,c.organization FROM users u
	LEFT JOIN creditcard c ON u.userid = c.userid;
    
-- Select all Users
select * from users;

-- Select all products with their corresponding store names

SELECT p.name as ProductName , s.name as Store from product p
JOIN store s
ON p.sid = s.sid;

-- Select all orders and their total amounts

SELECT orderNumber, SUM(totalAmount) as TotalAmount from orders
...
-- Select all comments with a grade above 4.0

SELECT content, grade FROM comments
WHERE grade >= 4.0;

-- Count the number of products in each store

SELECT s.name StoreName, COUNT(p.brand) ProductCount FROM product p
LEFT JOIN store s 
ON p.sid = s.sid
GROUP BY s.name;

-- Calculate the total amount from order

SELECT orderNumber, SUM(totalAmount) as TotalAmount FROM orders
GROUP BY orderNumber
ORDER BY TotalAmount DESC;

-- Calculate the total amount spent in each order

SELECT o.orderNumber, SUM(oi.price * c.quantity) as TotalAmount FROM orders o
JOIN contain c ON o.orderNumber = c.orderNumber
JOIN orderitem oi ON oi.itemid =  c.itemid
GROUP BY o.orderNumber
ORDER BY TotalAmount DESC;

-- Get the average rating of products:

SELECT AVG(grade) AS AverageRating 
FROM Comments;

-- Get a list of all users who have made comments

SELECT u.name from users u
JOIN comments c ON u.userid = c.userid;

-- List all products with their associated brand names

SELECT p.name ProductName, b.brandName BrandName FROM product p
JOIN brand b ON p.brand = b.brandName;

-- Find users who have saved at least one product to their shopping cart

SELECT u.name FROM users u
JOIN Save_to_Shopping_Cart sc  ON u.userid = sc.userid
GROUP BY u.name
HAVING COUNT(sc.pid) > 0;

-- Retrieve orders along with delivery addresses:

SELECT o.orderNumber, a.streetaddr, a.city 
FROM Orders o 
JOIN Deliver_To d ON o.orderNumber = d.orderNumber 
JOIN Address a ON d.addrid = a.addrid;

-- Get the total payments made for each order:

SELECT o.orderNumber, SUM(p.payTime) AS TotalPayments
FROM Orders o 
